  ...
  return (
    <RevelarScroll>
      ...
      <S icono="conversacion" titulo="Conversaciones (para todos)">
        ...
        <li><strong>Hay una conversación general</strong> para lo que cruza a varios equipos: cortes de transporte, faltantes, pedir una mano a otra área. La lee <strong>toda la organización</strong>, así que lo de una solicitud concreta —direcciones, teléfonos, nombres— sigue yendo en la conversación de esa solicitud. Se abre desde la pastilla «General» del panel.</li>
        <li><strong>Stickers.</strong> Con el botón del corazón, junto a «Enviar», mandas «Voy en camino», «Recibido», «Hecho»… Son ocho y están hechos para decir lo de siempre sin escribirlo.</li>
        <li>En <strong>«Conversaciones»</strong> (menú lateral) tienes todas las tuyas juntas, con lo que llevas sin leer. Y abajo a la derecha, en cualquier pantalla, hay un <strong>botón redondo</strong> que abre las conversaciones <strong>encima de lo que estés haciendo</strong>: lees, respondes y lo cierras, sin perder la pantalla de debajo. Si tiene un número, son los mensajes que te esperan.</li>
      </S>
      ...
      {f.gestionCasos && !f.verificacion && (
        <S icono="documento" titulo="Gestión de solicitudes (tu función)">
          ...
          <li><strong>Elige el país al indicar la ubicación.</strong> La plataforma atiende <strong>Venezuela y Colombia</strong>. El desplegable de debajo cambia solo: en Venezuela pide el <strong>estado</strong>, en Colombia el <strong>departamento</strong> (y parroquia pasa a corregimiento, sector a barrio o vereda). Si te equivocas de país, la solicitud va al equipo equivocado y se le sugieren centros de acopio del otro lado de la frontera — revísalo antes de crear.</li>
          <li><strong>Desglosa lo que se necesita, ítem por ítem.</strong> En vez de escribir «colchones y comida» en un solo campo, añade cada cosa por separado con su <strong>cantidad</strong> y su <strong>unidad</strong>: «5 colchones», «20 raciones», «2 filtros de agua». Es lo que permite después seguir cada necesidad por su cuenta y saber qué se cubrió y qué no.</li>
          <li><strong>Los ítems se corrigen, en cualquier momento.</strong> Si te equivocaste o la necesidad cambió, edítalos: Verificación y Logística también pueden, y <strong>no hay fase límite</strong> — se puede con la entrega ya en marcha. Cada cambio queda <strong>registrado</strong> con quién lo hizo, cuándo y qué decía antes, así que corregir no es problema — es lo esperado.</li>
      ...
      {f.verificacion && (
        <S icono="ok" titulo="Verificación (tu función)">
          ...
          <li><strong>Revisa el desglose por ítem.</strong> La solicitud llega con lo que se necesita separado en ítems, cada uno con su cantidad. Corrige los que estén mal: <strong>tú también puedes editarlos</strong>, y cada cambio queda registrado con tu nombre. Un desglose bien puesto es lo que hace que Logística pueda medir después qué se cubrió de verdad.</li>
          ...
          <li><strong>Elige QUÉ ítems van a cada área.</strong> No hace falta mandar la solicitud entera a todos: al derivar marcas los ítems que le tocan a cada quien. Puedes mandar la comida a Alianzas y las medicinas a Redacción, por separado. Cada área verá <strong>solo lo suyo</strong>. Si no marcas nada, va la solicitud completa, como antes.</li>
      ...
      {f.acopio && (
        <S icono="camion" titulo="Logística (tu función)">
          ...
          <li><strong>Los centros del otro país salen marcados.</strong> Cúcuta está a unos 10 km de San Antonio del Táchira, así que por pura distancia un centro venezolano podría ser «el más cercano» de un caso colombiano. La plataforma pone <strong>siempre primero</strong> los centros del país de la solicitud; si aparece uno del otro lado, va etiquetado como <strong>«cruza frontera»</strong>. Verlo no es un permiso: usarlo implica aduanas y permisos, así que consúltalo con coordinación.</li>
          <li><strong>Cada ítem avanza por su cuenta.</strong> La solicitud ya no se mueve en bloque: cada ítem del desglose tiene su propio semáforo (Pendiente → En gestión → En ruta → Cumplido), y ese avance lo <strong>ven todas las áreas</strong>, así que nadie tiene que preguntar por WhatsApp cómo va.</li>
          <li><strong>Registra el cumplimiento con números.</strong> «4 de 5 colchones» se anota tal cual: cuánto se cubrió, <strong>quién lo aportó</strong> y de dónde salió (inventario, un proveedor, una donación o un miembro del equipo). Se acabó el «casi completo».</li>
          <li><strong>Si lo cubrió otro, márcalo como tal.</strong> Cuando otra ONG o un particular cubre un ítem, márcalo <strong>cumplido por un tercero</strong> con su nombre: deja de requerir gestión nuestra. Ojo: eso <strong>no cuenta como capacidad nuestra</strong> en los reportes, y así debe ser — lo que hizo otro es de otro.</li>
          <li><strong>Entrega:</strong> avanza los estados (Solicitado → En gestión → En ruta → Entregado) y adjunta la <strong>evidencia (foto/nota)</strong>. El caso se cierra como <em>resuelto</em>. Si entregas solo una parte, márcalo como <strong>entrega parcial</strong>: el caso <strong>no</strong> se da por resuelto y sigue en difusión para cubrir lo que falta.</li>
          <li><strong>Corrige el desglose sin salir de la gestión.</strong> En el detalle de la solicitud, en <strong>«Corregir el desglose»</strong>, puedes <strong>agregar, cambiar o quitar</strong> ítems <strong>en cualquier fase</strong> — también con la entrega en marcha. Aparece un gasto de traslado que nadie previó, la familia ya consiguió los colchones, el número real era otro: eso pasa, y hay que poder anotarlo. Lo que añadas entra en tu parte del caso, así que lo seguirás viendo ahí.</li>
          <li><strong>Una solicitud vieja también se puede desglosar.</strong> Las anteriores al ítem solo tenían un texto suelto («50 cajas de agua»). Ábrela y sepárala en líneas: el texto de siempre se recalcula solo. No hace falta que nazca desglosada para adoptarla.</li>
          <li><strong>Para quitar un ítem con entregas anotadas, cancélalo.</strong> Borrar se llevaría por delante el registro de lo que sí se entregó y la cobertura de los reportes cambiaría hacia atrás. Un ítem <strong>cancelado</strong> se queda a la vista con su historial intacto. Borrar de verdad queda para el error de tecleo recién cometido.</li>
          <li><strong>Tus propias solicitudes, completas.</strong> Cuando registras una solicitud desde tu panel, ahora lleva lo mismo que las demás: contacto, ubicación, adjuntos y su desglose por ítem. Antes nacían sin nada de eso y luego no se podían cerrar bien.</li>
          <li><strong>Con qué aliados puedes contar.</strong> En <strong>«Proveedores»</strong> ves lo que cada aliado se comprometió a dar y —lo importante— <strong>cuánto le queda</strong>. Si una panadería ofrece 50 comidas semanales y ya se le pidieron 12, ves 38 disponibles; la semana siguiente vuelve a 50 sola. Lo puntual se agota y no vuelve.</li>
          <li><strong>Reportería del área:</strong> en <strong>«Reportería Logística»</strong> tienes cuánto se pide y de qué tipo, la cobertura real, <strong>en cuánto tiempo se entrega</strong> y quién sostiene la respuesta. Descargable e imprimible.</li>
      ...
      {f.alianzas && (
        <S icono="enlace" titulo="Alianzas Estratégicas (tu función)">
          ...
          <li><strong>Declara qué puede dar cada aliado.</strong> En <strong>«Aliados y capacidad»</strong> anotas qué cubre cada empresa, <strong>cuánto</strong> y <strong>cada cuánto</strong>: una sola vez, por tiempo limitado, o cada semana, quincena, mes o trimestre. Eso es exactamente lo que Logística lee como capacidad disponible, así que sé preciso: si dices «50 comidas semanales», Logística va a contar con 50 cada semana.</li>
          <li><strong>Deja claro lo puntual.</strong> Una donación de una sola vez se agota y no vuelve; una con fecha de vencimiento marca cero cuando caduca, con su motivo. Agotarse y vencerse piden decisiones distintas, y Logística necesita saber cuál es cuál.</li>
          <li><strong>Historial de cada empresa.</strong> En la ficha del aliado ves cuánto ha aportado de verdad, desde cuándo, en qué, y <strong>lo prometido frente a lo entregado</strong>. Es el dato con el que se decide a quién volver a pedirle.</li>
          <li><strong>Correo institucional:</strong> en <strong>«Correo institucional»</strong> escribes con las <strong>plantillas</strong> del departamento —presentación, solicitud de donación, agradecimiento, seguimiento— y <strong>cada envío queda registrado</strong>: a quién, con qué plantilla y si llegó o falló. Sale siempre desde la dirección de la organización, no desde tu correo personal.</li>
          <li><strong>Puedes ver Logística</strong> (solo lectura) para saber en qué se está usando lo que consigues. Ellos ven tu panel igual, sin los datos de contacto personales de la Ficha de Prospección: <strong>eso no sale del área</strong>.</li>
